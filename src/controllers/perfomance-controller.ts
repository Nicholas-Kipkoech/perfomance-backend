...
ORDER BY cm_order_no
  ...
  async getCMLossRatio2(req: Request, res: Response) {
    let connection;
    let results;
    try {
      const fromDate: string | any = req.query.fromDate;
...
GROUP BY BACNT_BANK_CODE,
  ...
  async getBusinessSummaryPerBranch(req: Request, res: Response) {
    ...
    try {
      ...
      // Construct SQL query with conditional parameter inclusion
      let query = ` 
  /* Formatted on 8/14/2024 6:08:30 PM (QP5 v5.336) */
  SELECT pr_org_code,
         pr_os_code,
         os_name,
             ...
             WHERE     trn_org_code = :p_org_code
                   ...
                   AND TRUNC (trn_doc_gl_dt) BETWEEN TRUNC (
                                                 ...
                                                 AND TRUNC (
                                                         NVL ( :p_to_dt,
                                                              trn_doc_gl_dt))
          GROUP BY trn_org_code, trn_os_code, ent_os_code)
GROUP BY pr_org_code, pr_os_code, os_name
  ...
  SELECT branch_code,
         ...
         SUM (claim_paid)
    FROM (  SELECT a.ent_os_code                                                branch_code,
              ...
              FROM (  SELECT DISTINCT
                        ...
                        FROM cm_payments_vw a,
                             vw_premium_register c,
                             (SELECT DISTINCT
                                     NVL (os_org_code, :p_org_code)
                                         os_org_code,
                                     NVL (os_code, '100')
                                         os_code,
                                     NVL (os_name, 'Un-Assigned')
                                         os_name,
                                     NVL (ent_os_code, '100')
                                         ent_os_code,
                                     NVL (
                       ...
                       WHERE     a.cm_int_aent_code = b.ent_aent_code
                             ...
                             AND a.cm_end_index = c.pr_end_index
                             AND a.hd_cur_code = NVL ( :p_currency, a.hd_cur_code)
                             AND TRUNC (a.hd_gl_date) BETWEEN NVL (
                                                                  :p_fm_dt,
                                                                  TRUNC (
                                                                      a.hd_gl_date))
                                                          AND NVL (
